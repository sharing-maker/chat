...
import type { ChatInputProps } from "../types/chat" // Import type from chat.ts
...
export function ChatInput({
  ...
  const [textToolbarStyle, setTextToolbarStyle] = useState<React.CSSProperties>({})
  ...
  const calculatePopupPosition = useCallback(
    (
      ...
      setPopupStyle: React.Dispatch<React.SetStateAction<React.CSSProperties>>,
    ) => {
          ...
          <div className="px-4 pt-3 pb-2 border-b border-gray-100 flex flex-wrap gap-2">
            {selectedFiles.map((file, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex items-center bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-1 rounded-lg relative overflow-hidden"
                >
                  {isImage ? (
                    <img src={fileUrl || ""} alt={file.name} className="h-12 w-12 object-cover rounded-md mr-2" />
                  ) : (
                  ...
                  <button
                    ...
                    className="ml-1 p-0.5 rounded-full hover:bg-blue-200 text-blue-600 absolute top-0.5 right-0.5 bg-white/70 backdrop-blur-sm"
        ...
        <div className="px-4 py-3">
          <form onSubmit={handleSubmit} className="flex items-end space-x-3">
            ...
            <button
              ...
              aria-label={isUploading ? "Đang tải lên" : "Gửi tin nhắn"}
            >
          ...
          <div className="flex items-center space-x-2 justify-start">
            {actionButtons.map((button, index) => {
              ...
              return (
                <button
                  key={index}
                  ref={button.ref} // Assign ref here
