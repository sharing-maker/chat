...
// Define the props interface as specified
...
export function ImagePreviewModal({ images, initialImageId, onClose }: ImagePreviewModalProps) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onClose}
        className="absolute top-4 right-4 text-white p-2 rounded-full bg-gray-800/50 hover:bg-gray-700/70 transition-colors z-50"
        aria-label="Đóng"
        title="Đóng (Esc)"
      >
        <X className="w-6 h-6" />
      ...
      {/* Image Container */}
      <div
        className="relative flex items-center justify-center w-full h-full max-w-screen-xl max-h-screen-xl"
      ...
      >
        ...
        <button
          ...
          className={`absolute left-2 sm:left-4 p-3 rounded-full bg-gray-800/50 text-white hover:bg-gray-700/70 transition-colors z-40
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          <Image
            ...
            alt={currentImage.name || "Xem trước hình ảnh"}
            layout="fill" // Use fill to make it responsive within its parent
            objectFit="contain" // Ensure the image fits within the container without cropping
            className="rounded-lg shadow-xl"
            priority // Load immediately for better UX
          />
        </div>
        ...
        <button
          ...
          className={`absolute right-2 sm:right-4 p-3 rounded-full bg-gray-800/50 text-white hover:bg-gray-700/70 transition-colors z-40
